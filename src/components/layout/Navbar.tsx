...
export function Navbar({ site }: { site: SiteSettings }) {
  ...
  return (
    <>
      <header
        className={`fixed inset-x-0 top-0 z-40 h-[54px] transition-colors duration-300 ${tone}`}
      >
        <div className="relative mx-auto h-full w-full max-w-[1440px]">
          {/* Left: menu trigger + links (Figma: x 75, y 11) */}
          <div className="absolute left-[75px] top-[11px] flex items-center gap-[26px]">
            <button
              type="button"
              aria-label="Open menu"
              aria-expanded={menuOpen}
              onClick={() => setMenuOpen(true)}
              className={`flex h-[32px] w-[40px] items-center justify-center ${scrolled ? "text-bark" : "text-cream"}`}
            >
              <Icon src="/icons/star-white.svg" width={40} height={32} />
            </button>
            {site.nav.left.map((l) => (
              <Link key={l.label} href={l.href} className="t-nav whitespace-nowrap">
                {l.label}
              </Link>
            ))}
          </div>
          ...
          <div className="absolute right-[56px] top-[18px] flex h-[20px] items-center justify-end gap-[11px]">
            {site.nav.right.map((l) => (
              <Link key={l.label} href={l.href} className="t-nav whitespace-nowrap">
                {l.label}
              </Link>
            ))}
            <a href={`tel:${site.contact.phone}`} aria-label="Call us" className="flex">
              <Icon src="/icons/phone.svg" width={14} height={14} />
            </a>
            <a href={`mailto:${site.contact.email}`} aria-label="Email us" className="flex">
